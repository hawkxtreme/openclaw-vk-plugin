...
- published on `main`
- install flow works from a prepared local bundle
...
## Priority 2: Re-run group chat smoke

Goal: close the remaining group-chat gap on the standalone repo.

Verify:

- community added to a real VK group chat
- mention-gated reply path
- allowlist or pairing policy behavior for group traffic

Done when:

- one real group mention produces a correct reply
- mention gating behaves as configured
- group policy behavior is documented in `docs/LIVE-VERIFICATION.md`

## Priority 3: Final security and review pass
...
## Priority 4: Docker polish for ordinary users

Goal: reduce surprise for users who just want the plugin to work quickly.

Work:

- document the expected duplicate-plugin warning clearly
- document the recommended standalone Docker install path
- decide whether a dedicated image path is worth adding later

Done when:

- a user can understand the warning and continue confidently
- Docker instructions are copy-paste-friendly and short

## Priority 5: Release polish
...
2. Group-chat smoke
3. Security and code review pass
4. Docker/docs polish
5. Final verification and release polish
